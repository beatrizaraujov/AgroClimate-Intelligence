        ...
        <div className="hidden md:flex absolute top-6 left-6 z-10 max-w-[240px] bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-2xl shadow-2xl transition-all hover:scale-105">
          <div className="flex gap-3">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[8px] font-bold text-teal-200 uppercase tracking-tighter">Status Atual</span>
              <h2 className="text-white text-xs font-bold leading-tight">
                {estadoAtivo ? `Análise: Estado ${estadoAtivo}` : "Arco do Desmatamento"}
              </h2>
              <p className="text-[9px] text-teal-100/60 leading-normal mt-1">
                {estadoAtivo 
                  ? `Cruzando dados do Sentinel-2 para a região de ${estadoAtivo}.`
        ...
        <div className="w-full h-full flex items-center justify-center p-2 md:p-4">
          ...
          >
            {mapData.map((state) => {
              ...
              const isSelected = estadoAtivo === state.id;
              ...
              return (
                <path
                  ...
                  className={`
                    cursor-pointer transition-all duration-500 ease-in-out stroke-[#2D7A73]
                    ${isSelected 
                      ? "fill-emerald-400 stroke-[3] drop-shadow-[0_0_15px_rgba(52,211,153,0.6)]" 
                      : "fill-teal-100/80 stroke-[1] md:stroke-[1.2] hover:fill-teal-50"
                    }
                  `}
                >
